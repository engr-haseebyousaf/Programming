...
include "connect.php";
$id = $_GET["uid"];
$show_sql = "SELECT * FROM `relationshipteacher` rt LEFT JOIN `relationshipStudent` rs ON rt.`teacherStudent`=rs.`studentId` LEFT JOIN `class_time` ct ON rt.`teacherClass`=ct.`classTimeId`  WHERE `teacherId` = '$id';";
...
if (isset($_POST["update"])) {
    ...
    if (!empty($teacherFirstName) && !empty($teacherLastName) && !empty($teacherPhone) && !empty($teacherEmail) && !empty($teacherStudent) && !empty($teacherClass)) {
        if (!empty($_FILES["teacherPic"]["name"])) {
            ...
            if (in_array($ext, $allowedExtensions)) {
                ...
                $sql = "UPDATE `relationshipTeacher` SET `teacherFirstName`='$teacherFirstName',`teacherLastName`='$teacherLastName',`teacherPhone`='$teacherPhone',`teacherEmail`='$teacherEmail',`teacherPic`='$teacherPicName' WHERE `teacherId` = $id;";
            ...
            $sql = "UPDATE `relationshipteacher` SET `teacherFirstName`='$teacherFirstName',`teacherLastName`='$teacherLastName',`teacherStudent`='$teacherStudent',`teacherClass`='$teacherClass',`teacherPhone`='$teacherPhone',`teacherEmail`='$teacherEmail' WHERE `teacherId` = $id;";
                if (mysqli_query($conn, $sql)) {
                    $msg = "<div class='alert alert-success'>Updated Successfully</div>";
                    header("Refresh: 1, ./index.php");
                }
    ...
    <div class="container py-3 bg-info">
        <form method="post" enctype="multipart/form-data">
            <div class="mb-3 row">
                ...
                <div class="col-8">
                    <input type="text" class="form-control" value="<?php echo $row["teacherFirstName"] ?>"
                        name="teacherFirstName" id="teacherFirstName" placeholder="teacherFirstName" />
                </div>
            </div>
            <div class="mb-3 row">
                <label for="teacherLastName" class="col-4 col-form-label"><b>teacherLastName</b></label>
                <div class="col-8">
                    <input type="text" class="form-control" value="<?php echo $row["teacherLastName"] ?>"
                        name="teacherLastName" id="teacherLastName" placeholder="teacherLastName" />
                </div>
            </div>
            <div class="mb-3 row">
                <label for="teacherStudent" class="col-form-label col-4"><b>teacherStudent</b></label>
                <div class="col-8">
                    <select class="form-select" name="teacherStudent" id="teacherStudent">
                        ...
                        while($teacher_row = mysqli_fetch_assoc($teacher_res)){
                            if ($teacher_row["studentId"] == $row["teacherStudent"]) {
                                $teacher_selected = "selected";
                            } else {
                                $teacher_selected = "";
                            }
                        ?>
                        <option value="<?php echo $teacher_row["studentId"] ?>" <?php echo $teacher_selected ?>>
                            <?php echo $teacher_row["studentFirstName"] ?> <?php echo $teacher_row["studentLastName"] ?>
                        </option>
                        <?php
                        }
                        ?>
            ...
            <div class="mb-3 row">
                <label for="" class="col-4 col-form-label"><b>teacherClass</b></label>
                <div class="col-8">
                    <select class="form-select" name="teacherClass" id="teacherClass">
                        ...
                        while ($class_row = mysqli_fetch_assoc($class_res)) {
                            if ($class_row["classTimeId"] == $row["teacherClass"]) {
            ...
            <div class="mb-3 row">
                ...
                <div class="col-8">
                    <input type="text" class="form-control" value="<?php echo $row["teacherPhone"] ?>"
                        name="teacherPhone" id="teacherPhone" placeholder="teacherPhone" />
                </div>
            </div>
            <div class="mb-3 row">
                <label for="teacherEmail" class="col-4 col-form-label"><b>teacherEmail</b></label>
                <div class="col-8">
                    <input type="email" class="form-control" value="<?php echo $row["teacherEmail"] ?>"
                        name="teacherEmail" id="teacherEmail" placeholder="teacherEmail" />
                </div>
            </div>
