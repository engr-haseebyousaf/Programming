...
include "config.php";
$id = $_GET["uid"];
$select_sql = "SELECT * FROM `class_time` WHERE `classTimeId`=$id";
...
if (isset($_POST["update"])) {
    ...
    $sql = "UPDATE `class_time` SET `classTime`='$classTime',`classTimeDescription`='$classTimeDescription'";
    // die($sql);
    ...
    <div class="container py-3 mt-3 bg-warning">
        <form method="post">
            <div class="mb-3 row">
                ...
                <div class="col-8">
                    <input type="time" required value="<?php echo $row["classTime"] ?>" class="form-control" name="classTime" id="classTime" placeholder="classTime" />
                </div>
            </div>
            <div class="mb-3 row">
                <label for="classTimeDescription" class="col-4 col-form-label"><b>classTimeDescription</b></label>
                <div class="col-8">
                    <input type="text" required value="<?php echo $row["classTimeDescription"] ?>" class="form-control" name="classTimeDescription" id="classTimeDescription" placeholder="classTimeDescription" />
                </div>
            </div>
